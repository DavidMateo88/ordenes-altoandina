...
async function loadProyectosForFilter() {
  try {
    const response = await fetch('https://gestion-altoandina.onrender.com/api/proyectos', {
      headers: { 'Authorization': `Bearer ${token}` }
    });
    if (!response.ok) {
      const error = await response.json();
...
async function editProyecto() {
  ...
  try {
    const response = await fetch(`https://gestion-altoandina.onrender.com/api/proyectos/${id}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`
